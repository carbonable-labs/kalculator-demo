...
export const advice_timeline = (timeConstraints: TimeConstraint): Advice => {
    if (timeConstraints === TimeConstraint.Yearly) {
        return { change: true, advice: "You should consider a five-year timeframe to avoid spot financing." };
    }
...
export const advice_typo = (typology: Typology, typoCosts: TypologyCosts): Advice => {
    let totalCost = typoCosts.costBiochar + typoCosts.costDac + typoCosts.costNbsAvoidance + typoCosts.costNbsRemoval;
...
export const advice_geography = (region: RegionAllocation, regionsCosts: RegionCosts): Advice => {
    let totalCost = regionsCosts.africa + regionsCosts.asia + regionsCosts.europe + regionsCosts.northAmerica + regionsCosts.oceania + regionsCosts.southAmerica;
